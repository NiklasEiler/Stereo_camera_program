...
class Bildverarbeitung:
    def __init__(self, Img):
        ...
        self.solution_roa= np.array((250,300))
    ...
    def binar(self):   #Otsu mit Gaußfilterung         
        ret ,self.imgbin= cv2.threshold(cv2.bitwise_not(self.blur),0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        
      
    def speichern(self):
        df_data= pd.DataFrame(self.data, columns=['Fläche', 'Radius'])
    ...
    def regionofattraction(self):
        self.binar()
        self.find_large_regions(100, 100000)
        self.flaechen_mittelpunkt()
        if all(self.mittekoor) != None:
            # Berechne die Koordinaten des Ausschnitts
            y = int( self.mittekoor[0] - self.solution_roa[0]/2)
            x = int(self.mittekoor[1] - self.solution_roa[1]/2)
            if y < 0 and x > 0:
                self.regionofattraction_img= self.img[0:abs(y)+self.solution_roa[0], x:x+self.solution_roa[1]]
            elif y > 0 and x < 0:
                self.regionofattraction_img = self.img[y:y+self.solution_roa[0], 0:abs(x)+self.solution_roa[1]]
            elif y < 0 and x < 0:
                self.regionofattraction_img = self.img[0:abs(y)+self.solution_roa[0], 0:abs(x)+self.solution_roa[1]]
            else:
               self.regionofattraction_img = self.img[y:y+self.solution_roa[0], x:x+self.solution_roa[1]] 
